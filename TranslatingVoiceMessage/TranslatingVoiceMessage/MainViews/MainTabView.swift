...
struct MainTabView: View {
    ...
    enum Tab: String, CaseIterable {
        ...
        var label: String {
            switch self {
            case .voice:    return L(.tab_translate)
            case .keyboard: return L(.tab_keyboard)
            case .settings: return L(.tab_settings)
            }
        }
    }
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            Group {
                switch selectedTab {
                case .voice:    VoiceTranslationView(autoStartListening: $openedFromKeyboard)
                case .keyboard: PermissionInstructionView()
    ...
    private var customTabBar: some View {
        HStack(spacing: 0) {
            ForEach(Tab.allCases, id: \.rawValue) { tab in tabButton(for: tab) }
        }
        .padding(.horizontal, 8)
        .padding(.top, 10)
        .padding(.bottom, 24)
        ...
        Button {
            ...
            VStack(spacing: 5) {
                ZStack {
                    if selectedTab == tab {
                        Capsule()
                            .fill(LinearGradient(colors: [accentBlue.opacity(0.25), accentPurple.opacity(0.15)], startPoint: .leading, endPoint: .trailing))
                            .frame(width: 56, height: 30)
                            .transition(.scale.combined(with: .opacity))
                    }
                    Image(systemName: tab.icon)
                        ...
                        .foregroundStyle(
                            selectedTab == tab
                            ? AnyShapeStyle(LinearGradient(colors: [accentBlue, accentPurple], startPoint: .topLeading, endPoint: .bottomTrailing))
                            : AnyShapeStyle(Color.white.opacity(0.3))
                        )
                        .scaleEffect(selectedTab == tab ? 1.0 : 0.9)
                }
    ...
    private var tabBarBackground: some View {
        ZStack {
            Rectangle().fill(tabBarColor.opacity(0.92))
            VStack {
                LinearGradient(colors: [accentBlue.opacity(0.15), accentPurple.opacity(0.08), .clear], startPoint: .leading, endPoint: .trailing)
                    .frame(height: 1)
                Spacer()
            }
        }
...
struct SettingsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            GeometryReader { geo in
                Circle().fill(accentPurple.opacity(0.06)).frame(width: 250, height: 250).blur(radius: 70).offset(x: geo.size.width - 100, y: 50)
            }
                        ...
                        HStack(spacing: 14) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 14)
                                    ...
                                    .frame(width: 52, height: 52)
                                Image(systemName: "globe").font(.system(size: 24, weight: .medium)).foregroundColor(.white)
                            }
                            VStack(alignment: .leading, spacing: 3) {
                                Text(L(.settings_app_name)).font(.system(size: 17, weight: .bold, design: .rounded)).foregroundColor(.white)
                                Text(L(.settings_version)).font(.system(size: 12)).foregroundColor(.white.opacity(0.4))
                            }
                            Spacer()
                        }
                    }

                    sectionHeader(L(.settings_section_features))

                    settingsCard {
                        VStack(spacing: 0) {
                            featureRow(icon: "mic.fill",              color: accentBlue,   title: L(.settings_feature_voice),    subtitle: L(.settings_feature_voice_sub))
                            Divider().background(Color.white.opacity(0.06)).padding(.leading, 48)
                            featureRow(icon: "keyboard",              color: accentPurple, title: L(.settings_feature_keyboard), subtitle: L(.settings_feature_keyboard_sub))
                            Divider().background(Color.white.opacity(0.06)).padding(.leading, 48)
                            featureRow(icon: "square.and.arrow.up",   color: accentGreen,  title: L(.settings_feature_share),    subtitle: L(.settings_feature_share_sub))
                        }
                    }

                    sectionHeader(L(.settings_section_translation))

                    settingsCard {
                        VStack(spacing: 0) {
                            infoRow(icon: "globe.americas",  title: L(.settings_info_languages),       value: L(.settings_info_languages_value))
                            Divider().background(Color.white.opacity(0.06)).padding(.leading, 48)
                            infoRow(icon: "bolt.fill",        title: L(.settings_info_speech_engine),   value: L(.settings_info_speech_engine_value))
                            Divider().background(Color.white.opacity(0.06)).padding(.leading, 48)
                            infoRow(icon: "network",          title: L(.settings_info_translation_api), value: L(.settings_info_translation_api_value))
                        }
                    }

                    sectionHeader(L(.settings_section_help))

                    settingsCard {
                        VStack(spacing: 0) {
                            Button { openAppSettings() } label: {
                                settingsRow(icon: "gearshape.fill",       color: .gray,      title: L(.settings_help_system_settings))
                            }
                            Divider().background(Color.white.opacity(0.06)).padding(.leading, 48)
                            settingsRow(icon: "questionmark.circle", color: accentBlue, title: L(.settings_help_how_to_use))
                        }
                    }

                    Text(L(.settings_footer))
                        .font(.system(size: 11))
                        .foregroundColor(.white.opacity(0.2))
                        .multilineTextAlignment(.center)
                        .frame(maxWidth: .infinity)
    ...
    private func sectionHeader(_ title: String) -> some View {
        Text(title).font(.system(size: 12, weight: .bold)).foregroundColor(.white.opacity(0.3)).padding(.leading, 4)
    }
    ...
    private func featureRow(icon: String, color: Color, title: String, subtitle: String) -> some View {
        HStack(spacing: 14) {
            ...
            Spacer()
            Image(systemName: "checkmark.circle.fill").font(.system(size: 16)).foregroundColor(accentGreen)
        }
        .padding(.vertical, 6)
    }
    ...
    private func infoRow(icon: String, title: String, value: String) -> some View {
        HStack(spacing: 14) {
            ZStack {
                RoundedRectangle(cornerRadius: 8).fill(Color.white.opacity(0.06)).frame(width: 34, height: 34)
            ...
            Spacer()
            Text(value).font(.system(size: 13, weight: .medium)).foregroundColor(.white.opacity(0.4))
        }
        .padding(.vertical, 6)
    }
    ...
    private func settingsRow(icon: String, color: Color, title: String) -> some View {
        HStack(spacing: 14) {
            ...
            Text(title).font(.system(size: 14, weight: .medium)).foregroundColor(.white)
            Spacer()
            Image(systemName: "chevron.right").font(.system(size: 12, weight: .bold)).foregroundColor(.white.opacity(0.2))
        }
        .padding(.vertical, 6)
    }
