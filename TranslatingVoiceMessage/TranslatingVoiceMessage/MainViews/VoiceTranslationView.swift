...
// ============================================================
//  VoiceTranslationView.swift  (LOCALIZED)
//  TranslatingVoiceMessage
//
//  Changes from original:
//    • All hardcoded strings replaced with L(.key) / L(.key, arg)
...
// MARK: - Speech Manager (unchanged — no UI strings here)
...
class SpeechManager: ObservableObject {
        ...
        SFSpeechRecognizer.requestAuthorization { status in
            DispatchQueue.main.async {
                ...
                case .authorized:
                    completion(true)
                default:
                    self.errorMessage = L(.error_permission_denied)
                    completion(false)
                }
            }
        }
    ...
    func startListening() {
        guard let recognizer = speechRecognizer, recognizer.isAvailable else {
            errorMessage = L(.error_speech_unavailable)
            return
        }
        ...
        do {
            ...
            errorMessage = L(.error_audio_session, error.localizedDescription)
            return
        }
        ...
        recognitionTask = recognizer.recognitionTask(with: request) { [weak self] result, error in
            DispatchQueue.main.async {
                ...
                if let error = error, self?.isListening == true {
                    self?.errorMessage = error.localizedDescription
                }
            }
        }
        ...
        do {
            ...
            errorMessage = L(.error_audio_engine, error.localizedDescription)
        }
    }
...
struct VoiceTranslationView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if showLanguagePicker { languagePickerOverlay }
        }
        .onAppear {
            loadLanguagePreference()
            speechManager.requestPermissions { granted in
        ...
        .onDisappear { speechManager.stopListening() }
    }
        ...
        VStack(spacing: 8) {
            ...
            if showFromKeyboardBanner {
                HStack(spacing: 6) {
                    Image(systemName: "keyboard")
                        .font(.system(size: 11, weight: .medium))
                    Text(L(.voice_banner_from_keyboard))
                        .font(.system(size: 12, weight: .medium))
                }
                .foregroundColor(.white)
                .padding(.horizontal, 14)
                ...
                .background(
                    Capsule()
                        .fill(accentBlue.opacity(0.3))
                        .overlay(Capsule().stroke(accentBlue.opacity(0.4), lineWidth: 1))
                )
                .transition(.move(edge: .top).combined(with: .opacity))
            }
        }
    ...
    private var languageSelector: some View {
        Button {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(L(.voice_translate_to_label))
                        .font(.system(size: 10, weight: .medium))
                        .foregroundColor(.white.opacity(0.4))
                    Text(selectedLanguage.name)
                        .font(.system(size: 15, weight: .bold, design: .rounded))
        ...
        VStack(spacing: 14) {
            ZStack {
                if speechManager.isListening {
                    ForEach(0..<3, id: \.self) { i in
                        Circle()
                            ...
                            .opacity(speechManager.isListening ? 0 : 0.6)
                            .animation(
                                .easeInOut(duration: 1.5).repeatForever(autoreverses: false).delay(Double(i) * 0.3),
                                value: speechManager.isListening
                            )
                    }
                }
                ...
                Button {
                    if speechManager.isListening {
                        speechManager.stopListening()
                        if !speechManager.transcribedText.isEmpty { translateText() }
                    } else {
                        translatedText = ""
                        speechManager.transcribedText = ""
                        speechManager.startListening()
    ...
    private var statusText: String {
        if let error = speechManager.errorMessage { return "⚠️ \(error)" }
        if speechManager.isListening { return L(.voice_status_listening) }
        if !speechManager.transcribedText.isEmpty { return L(.voice_status_tap_to_record) }
        return L(.voice_status_tap_to_start)
    }
    ...
    private var transcriptionCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text(L(.voice_label_original))
                    .font(.system(size: 10, weight: .bold))
                    .foregroundColor(.white.opacity(0.35))
                Spacer()
                if !speechManager.transcribedText.isEmpty {
                    Button { UIPasteboard.general.string = speechManager.transcribedText } label: {
                        Image(systemName: "doc.on.doc")
                            .font(.system(size: 11))
                            .foregroundColor(.white.opacity(0.3))
                    }
            ...
            if speechManager.transcribedText.isEmpty {
                Text(L(.voice_placeholder_speech))
                    .font(.system(size: 15))
                    .foregroundColor(.white.opacity(0.25))
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.vertical, 4)
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 16)
                        .stroke(speechManager.isListening ? Color.red.opacity(0.3) : Color.white.opacity(0.05), lineWidth: 1)
                )
        )
    }
    ...
    private var translationCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                HStack(spacing: 6) {
                    Text(L(.voice_label_translated))
                        .font(.system(size: 10, weight: .bold))
                        .foregroundColor(accentGreen.opacity(0.7))
                    if !translatedText.isEmpty { Text(selectedLanguage.flag).font(.system(size: 12)) }
                }
                Spacer()
                if isTranslating { ProgressView().scaleEffect(0.7).tint(accentGreen) }
            }

            if translatedText.isEmpty && !isTranslating {
                Text(L(.voice_placeholder_translation))
                    .font(.system(size: 15))
                    .foregroundColor(.white.opacity(0.25))
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.vertical, 4)
            } else if isTranslating {
                Text(L(.voice_translating, selectedLanguage.name))
                    .font(.system(size: 15, weight: .medium))
                    .foregroundColor(accentOrange)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.vertical, 4)
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 16)
                        .stroke(translatedText.isEmpty ? Color.white.opacity(0.05) : accentGreen.opacity(0.2), lineWidth: 1)
                )
        )
    }
    ...
    private var actionButtons: some View {
        VStack(spacing: 10) {
            if !translatedText.isEmpty {
                Button {
                    ...
                    HStack(spacing: 10) {
                        Image(systemName: copied ? "checkmark.circle.fill" : "doc.on.doc.fill")
                            .font(.system(size: 16, weight: .semibold))
                        Text(copied ? L(.voice_copied) : L(.voice_copy_translation))
                            .font(.system(size: 16, weight: .bold, design: .rounded))
                    }
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 16)
                    .background(
                        RoundedRectangle(cornerRadius: 14)
                            .fill(
                                copied
                                ? AnyShapeStyle(accentGreen)
                                : AnyShapeStyle(LinearGradient(colors: [accentBlue, accentPurple], startPoint: .leading, endPoint: .trailing))
                            )
                    )
                    .foregroundColor(.white)
                    .shadow(color: accentBlue.opacity(0.3), radius: 10, y: 4)
            ...
            if !speechManager.transcribedText.isEmpty && translatedText.isEmpty && !isTranslating && !speechManager.isListening {
                Button { translateText() } label: {
                    HStack(spacing: 8) {
                        Image(systemName: "globe").font(.system(size: 16, weight: .semibold))
                        Text(L(.voice_translate_to, selectedLanguage.name))
                            .font(.system(size: 15, weight: .bold, design: .rounded))
                    }
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 14)
                    .background(
                        RoundedRectangle(cornerRadius: 14)
                            .fill(surfaceColor)
                            .overlay(RoundedRectangle(cornerRadius: 14).stroke(accentBlue.opacity(0.3), lineWidth: 1))
                    )
                    .foregroundColor(accentBlue)
                }
                .transition(.move(edge: .bottom).combined(with: .opacity))
            ...
            if !speechManager.transcribedText.isEmpty || !translatedText.isEmpty {
                Button {
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(L(.voice_clear)).font(.system(size: 13, weight: .medium))
                    }
                    .foregroundColor(.white.opacity(0.35))
                    .padding(.vertical, 8)
                }
                    ...
                    LazyVGrid(columns: [GridItem(.flexible(), spacing: 8), GridItem(.flexible(), spacing: 8)], spacing: 8) {
                        ForEach(SupportedLanguages.all, id: \.code) { lang in
                            Button {
                                ...
                                .padding(.horizontal, 14)
                                .padding(.vertical, 12)
                                .background(RoundedRectangle(cornerRadius: 10).fill(lang == selectedLanguage ? accentBlue.opacity(0.2) : surfaceColor))
                                .overlay(RoundedRectangle(cornerRadius: 10).stroke(lang == selectedLanguage ? accentBlue.opacity(0.4) : Color.clear, lineWidth: 1))
                            }
                        }
                    }
                    .padding(.horizontal, 16)
    ...
    private func translateText() {
        ...
        translationService.translate(text: text, to: selectedLanguage.code) { result in
            DispatchQueue.main.async {
                ...
                switch result {
                case .success(let translated):
                    withAnimation(.spring(response: 0.3)) { self.translatedText = translated }
                case .failure(let error):
                    self.translatedText = L(.error_translation_failed, error.localizedDescription)
                }
            }
        }
    }
